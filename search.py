...
def duffel_flights_list_offers():
    ...
    if not DUFFEL_API_KEY:
        return jsonify({"error": "Duffel API key not configured"}), 500
    attempt = 0
    while attempt < RETRIES:
        try:
            ...
            print(f"Duffel Response [{resp.status_code}]: {resp.text}")
            if resp.status_code >= 200 and resp.status_code < 300:
                return Response(resp.content, status=resp.status_code, content_type='application/json')
            else:
                attempt += 1
                if attempt >= RETRIES:
                    return jsonify({"error": "Duffel API error", "status_code": resp.status_code, "response": resp.text}), resp.status_code
